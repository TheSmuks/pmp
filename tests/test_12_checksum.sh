...
if [ -n "$_expected" ]; then
    ...
    _pike_hash="$(pike -M $_PIKE_M -e 'import Pmp.Helpers; write(compute_sha256("'"$TESTDIR"'" + "/pmp-test-sha.txt"));' 2>/dev/null)"
...
_dir_hash="$(pike -M $_PIKE_M -e 'import Pmp.Store; write(compute_dir_hash("'"$TESTDIR"'" + "/dir-test"));' 2>/dev/null)"
assert "compute_dir_hash produces 64-char hex" "64" "${#_dir_hash}"
# Verify determinism — same content, same hash
_dir_hash2="$(pike -M $_PIKE_M -e 'import Pmp.Store; write(compute_dir_hash("'"$TESTDIR"'" + "/dir-test"));' 2>/dev/null)"
